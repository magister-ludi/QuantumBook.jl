...
public struct Main extends Application
    ...
    public static Chart plotFunction(List<Function<Double, Double>> functions, Number xStart, Number xEnd)
        ...
        Axis<Number> xAxis = NumberAxis(x0, x1, .1 * (x1 - x0))
        Axis<Number> yAxis = NumberAxis()
        ObservableList<XYChart.Series<Number, Number>> series = FXCollections.observableArrayList()
        LineChart<Number, Number> chart = LineChart(xAxis, yAxis, series)
        chart.setCreateSymbols(false)
        int i = 0
        for (Function<Double, Double> f : functions)
            XYChart.Series<Number, Number> mainSeries = XYChart.Series()
